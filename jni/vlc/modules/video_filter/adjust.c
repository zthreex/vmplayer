...
static picture_t *FilterPlanar( filter_t *p_filter, picture_t *p_pic )
{
    ...
    i_cont = (int)( p_sys->f_contrast * 255 );   //1*255
    i_lum = (int)( (p_sys->f_brightness - 1.0)*255 );  //(1-1)*255
    f_hue = (float)( p_sys->i_hue * M_PI / 180 );  //0
    i_sat = (int)( p_sys->f_saturation * 256 ); //1*256
    f_gamma = 1.0 / p_sys->f_gamma; // 1/1
