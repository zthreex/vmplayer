...
#define CLASS com_zthreex_vmplayer_player_VlcMediaPlayer
#define PREFIX "com/zthreex/vmplayer/player/"
...
/*added by zx
 *1 modules all be removed
 *0 no
 * */
static int nIsRemoved = 0;
...
static inline vlc_jni_player_t *vlc_jni_player_find_or_throw(JNIEnv *env, jobject obj)
{
    ...
    {
        /* XXX: throw */
        __android_log_print(ANDROID_LOG_ERROR, "vmplayer", "could not find %p", obj);
    }
    return vj;
}
...
{
	//added
    __android_log_write(ANDROID_LOG_ERROR,"vmplayer_jni","async");  
...
JNIEXPORT void JNICALL NAME(nativeStart)(JNIEnv *env, jobject thiz)
{
    ...
    __android_log_write(ANDROID_LOG_ERROR,"vmplayer_jni","start");  
	nIsRemoved = 0;
//    __android_log_print(ANDROID_LOG_DEBUG, "vmplayer", "nIsRemoved : %d", nIsRemoved);
//added by zx,adjust the vmvideo YUV 
	libvlc_video_set_adjust_int(vj->player, libvlc_adjust_Enable, 1);
    libvlc_video_set_adjust_float(vj->player, libvlc_adjust_Gamma, 1.25);
}
...
JNIEXPORT int JNICALL NAME(nativeIsTotallyRemoved)(JNIEnv *env, jobject thiz) {
//    __android_log_print(ANDROID_LOG_DEBUG, "vmplayer", "STATUS FUNCTION CALLED");
	if(nIsRemoved)	return 1;	
	else return 0;
}


 /*added by zx
 * The thread is used to collect garbage.
 * It listens the player's status and do the things
 * */
        ...
        while (true)
        {
            for (int i = 0; i < vlc_array_count(s_VlcMediaPlayer_array); i++)
            {
                ...
                if (!status)
                {
                    ...
                    __android_log_print(ANDROID_LOG_DEBUG, "vmplayer", "about to destroy %p", vj->player);
                    break;
                }
            }
            if (status)
